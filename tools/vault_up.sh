...
docker compose --profile vault up -d --build --no-deps vault-certgen vault
wait_for_log vault "Vault server started" 180

docker compose --profile vault up -d --build --no-deps vault-bootstrap
wait_for_log vault-bootstrap "\[bootstrap\] DONE" 240

docker compose --profile vault up --force-recreate -d --build --no-deps vault-seed
