...
for file in \
  /run/secrets/grading_db_superuser_password \
  /run/secrets/grading_db_admin_password \
  /run/secrets/grading_db_app_password
do
  if [ ! -r "$file" ]; then
    echo "[grading-db] ERROR: missing or unreadable secret file: $file"
    exit 1
  fi
  echo "[grading-db] Found $file"
done

echo "[grading-db] Found all secrets. Starting init..."
...
psql -U postgres -c "CREATE USER $POSTGRES_SUPERUSER WITH PASSWORD '$(cat /run/secrets/grading_db_superuser_password)'"
...
psql -U postgres -c "CREATE USER $POSTGRES_ADMIN WITH PASSWORD '$(cat /run/secrets/grading_db_admin_password)'"
...
psql -U postgres -c "CREATE USER $POSTGRES_APP_USER WITH PASSWORD '$(cat /run/secrets/grading_db_app_password)'"
